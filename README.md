...
2. [Trust credentials](https://login.tailscale.com/admin/settings/trust-credentials) で
   Workload Identity Federation (OIDC) の federated identity を作成する:
   - Provider: GitHub Actions
   - Subject: `repo:ragujp/flets-v6:*`(このリポジトリからのみ許可)
   - Scope: `auth_keys` (write)
   - Tags: `tag:github-actions`
   - Client ID と Audience を控える(Secret は存在しない)。
...
| Secret | 内容 |
|---|---|
| `TS_OAUTH_CLIENT_ID` | Tailscale federated identity の Client ID |
| `TS_AUDIENCE` | Tailscale federated identity の Audience |
